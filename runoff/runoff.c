...
bool print_winner(void)
{
    for(int voters=0; voters <candidate_count; voters++){
        if(candidates[voters].votes>(candidate_count/2)){
            printf("%s\n",candidates[voters].name);
            return true;
        }
    }
...
int find_min(void)
{
    int min = MAX_VOTERS + 1; // Initialize with a value greater than the maximum number of voters
    for (int i = 0; i < candidate_count; i++) {
